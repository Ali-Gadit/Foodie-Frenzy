...
const handleStatusChange = async (orderId, newStatus) => {
    try {
        await axios.put(`${backendUrl}/api/orders/getall/${orderId}`,
        { status: newStatus });
                                    ...
                                    <tr key={order._id} className={tableClasses.row}>
                                            ...
                                            <div className='space-y-1'>
                                                {order.items.map((itm,idx)=>(
                                                    <div key={idx} className='flex items-center gap-3 p-2 rounded-lg'>
                                                        <img 
                                                            src={itm.item.imageUrl.startsWith('http') ? itm.item.imageUrl : `${backendUrl}${itm.item.imageUrl}`} 
                                                            alt={itm.item.name} 
                                        ...
                                        <td className={tableClasses.cellBase}>
                                            <div className='flex items-center gap-2'>
                                                ...
                                                onChange={e => handleStatusChange(order._id, e.target.value)} className={`px-4 py-2 rounded-lg ${stat.bg} ${stat.color} border border-amber-500/20 text-sm cursor-pointer`}>
