    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a120b] via-[#3c2a21] to-[#1a120b] text-amber-50 overflow-hidden relative">
          ...
          {features.map((f,i)=>{
            ...
            <div className="relative bg-[#3c2a21]/90 backdrop-blur-lg rounded-3xl overflow-hidden border border-amber-600/30 hover:border-amber-500 transition-all duration-300 h-full">
              <div className="relative h-64 overflow-hidden">                           
                <motion.img alt={f.title} className="w-full h-full object-cover" src={f.img} initial={{scale:1}} whileHover={{scale:1.05}} transition={{duration:0.4}} />
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((s,i)=>{
            ...
            whileInView={{opacity:1,y:0}} transition={{delay:i*0.2,type:'spring'}}
             className="relative group h-48" onHoverStart={()=>{setHoverStat(i)}}
            ...
            <motion.div className="absolute inset-0" animate={{
              ...
              <div className="relative h-full bg-[#3c2a21]/40 backdrop-blur-lg rounded-xl border-2 border-amber-600/30 p-6 overflow-hidden transition-all duration-300">
                ...
                <div className="relative z-10 h-full flex flex-col items-center justify-center">
                  ...
                  <div className="text-4xl font-bold mb-1 bg-gradient-to-r bg-clip-text from-amber-200 to-amber-400 text-transparent">
                    {s.number}
                  </div>
                  <motion.div className="text-sm uppercase tracking-widest font-medium text-amber-100/80" animate={{
                    letterSpacing:hoverStat === i ? '0.15em' : '0.1em',
                    textShadow:hoverStat === i ? '0 0 8px rgba(245,158,11,0.4)' : 'none'
                  }}>
                    {s.label}
                  </motion.div>
                </div>
      ...
      <section className="py-16 px-4 md:px-8 relative">
        <div className="max-w-7xl mx-auto">
          <motion.h2 className="text-4xl font-serif sm:text-5xl md:text-6xl font-bold text-center mb-12 text-amber-100" initial={{opacity:0,y:20}}
            ...
            {teamMembers.map((m,i)=>(
              ...
              <div className="relative h-full bg-[#3c2a21]/90 backdrop-blur-lg rounded-3xl overflow-hidden border-2 border-amber-600/30 hover:border-amber-500 transition-all duration-500 shadow-xl hover:shadow-2xl hover:shadow-amber-500/20">
                <div className="relative h-64 sm:h-72 md:h-96 overflow-hidden">
                  <motion.img alt={m.name} className="w-full h-full object-cover" src={m.img} initial={{scale:1}} whileHover={{scale:1.1}} transition={{duration:0.5}} />
                </div>
